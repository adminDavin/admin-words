...
class ManageTableTaskInfo extends React.Component {
      ...
      <div className="container">
        ...
        <div className="row">
          <button
            ...
            data-dismiss="modal"
            onClick={this.taskDoAction.bind(this, mine, "agree", item)}
          >
            {"审核通过"}
          </button>
          <button
            ...
            data-dismiss="modal"
            onClick={this.taskDoAction.bind(this, mine, "reject", item)}
          >
            {"审核拒绝"}
          </button>
        </div>
  ...
  taskDoAction(me, flag, item) {
    ...
    request.sendRequstNew(changUserStatue, params, function(result) {
      if (result.code != "200") {
        ...
        me.setState({ userId: 0, update: "finish", data: newData });
        setTimeout(() => {
          $("#loading").modal("hide");
        }, 500);
      }
  ...
  getTrContent(item, index) {
    ...
    let stateStyle =
      item.state == 0 ? "alert alert-secondary" : "alert alert-success";
    return (
      <tr key={index}>
        <td>{index + 1}</td>
        <td>{"待审核用户"}</td>
        <td>{"注册申请"}</td>
        <td>{utils.formatDate(item.applyDate)}</td>
        <td className={stateStyle}>{state}</td>
        <td>{item.email}</td>
        <td>
          <button
            type="button"
