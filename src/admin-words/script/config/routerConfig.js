...
const urlConfig ={
  viewer:{current:"/viewer.html",to:[{name:"userInfo",url:"/userInfo.html",button:title.userMange}],isUserAuth:true},
  userInfo:{current:"/userInfo.html",to:[{name:"viewer",url:"/viewer.html",button:title.viewer}],isUserAuth:true},  
  login:{current:"/login.html",to:[{name:"viewer",url:"/viewer.html",button:title.viewer},{name:"userInfo",url:"/userInfo.html",button:title.userMange}]}, 
  register:{current:"/register.html",to:[{name:"login",url:"/login.html",button:title.login}]}
};
...
const checkValidAndGetParams=function(config,params){
  ...
  config.to.map(function(item,index){
    urlGotoConfig.push({name:item.button,url:item.url,params:params,show:""});
  });
